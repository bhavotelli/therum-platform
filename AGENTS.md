...
2. Print the exact SQL the user needs to paste into the Supabase SQL editor, as a fenced ```sql block. Name the target project (prod vs. staging) if relevant — and if the user hasn't specified which project, ask before generating the SQL rather than guessing. Include RLS policies for any new table with agency data — see `.github/review-guidelines.md` for the required pattern.
