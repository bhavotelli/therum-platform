...
export default async function VatCompliancePage() {
  ...
  if (financeCtx.status === 'need_agency') {
    return (
      <div className="space-y-6">
        <h1 className="text-2xl font-bold text-zinc-900">VAT Compliance</h1>
  ...
  const statuses = await getVatMonitoringForAgency(financeCtx.agencyId)
    ...
    <div className="space-y-6">
      <header className="relative overflow-hidden rounded-2xl border border-gray-200 bg-white px-8 py-6 shadow-sm">
        ...
        <div className="relative">
          <h1 className="text-2xl font-bold tracking-tight text-gray-900">VAT Compliance</h1>
      ...
      {statuses.length === 0 ? (
        <div className="p-20 bg-zinc-50 rounded-2xl border-2 border-dashed border-zinc-200 flex flex-col items-center justify-center text-center">
          ...
          <p className="text-zinc-500 text-sm max-w-sm">All talent either have a VAT number set or are marked as VAT registered.</p>
        </div>
      ) : (
      ...
      <div className="rounded-lg border border-zinc-200 bg-zinc-50 px-4 py-3 text-xs text-zinc-600 space-y-1">
        ...
        <p className="text-zinc-400 pt-1">Earnings = net payout amounts from approved/paid invoices within the rolling 12-month window.</p>
      </div>
    </div>
  )
}
